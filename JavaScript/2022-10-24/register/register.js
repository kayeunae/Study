...
function idConfirm() {
    ...
    //userid가 공백이라면 (=빈 문자열이 됐을 때)
    //userid.value.repalce(/ |0/g,"") : 넓은 공백 또는 0 제거
    //userid.value.repalce(/ /g,"") : 넓은 공백 제거
...
function pwd2Confirm() {
    ...
    if(!pwd2.value.replace(/ /g," ")) {
        ...
        if(pwd1.value.replace(/ /g," ") && pwd2.value.replace(/ /g," ")) {  //두 패스워드 값이 있는지 확인 (있다면)
...
function telConfirm() {
    ...
    //전화번호가 있고 정규식(유효성) 체크에 통과하지 못했을 때
